...
<div class="anim-fade-up delay-1 flex justify-between items-start mb-6">
    <div>
        ...
        <h1 class="text-2xl font-black text-gray-900">Selamat Datang, {{ auth()->user()->name }}! 👋</h1>
...
<div class="grid grid-cols-4 gap-4 mb-6">
    ...
    <div class="stat-card anim-fade-up delay-2 bg-white rounded-2xl p-5 border border-gray-100"
        ...
        <p class="text-3xl font-black text-gray-900">{{ number_format($transaksiHariIni) }}</p>
    ...
    <div class="stat-card anim-fade-up delay-3 bg-white rounded-2xl p-5 border border-gray-100"
        style="box-shadow: 0 2px 12px rgba(0,0,0,0.04)">
        <div class="flex justify-between items-start mb-4">
            <div class="w-10 h-10 rounded-xl flex items-center justify-center"
        ...
        <p class="text-3xl font-black text-gray-900">{{ number_format($belumDiambil) }}</p>
        <div class="mt-3 h-1 bg-gray-100 rounded-full overflow-hidden">
            <div class="h-full rounded-full" style="width: 45%; background: linear-gradient(90deg, #ea580c, #fb923c)"></div>
        </div>
    </div>

    <div class="stat-card anim-fade-up delay-4 bg-white rounded-2xl p-5 border border-gray-100"
        style="box-shadow: 0 2px 12px rgba(0,0,0,0.04)">
        <div class="flex justify-between items-start mb-4">
            <div class="w-10 h-10 rounded-xl flex items-center justify-center"
        ...
        <p class="text-3xl font-black text-gray-900">{{ number_format($sudahDiambil) }}</p>
        <div class="mt-3 h-1 bg-gray-100 rounded-full overflow-hidden">
            <div class="h-full rounded-full" style="width: 80%; background: linear-gradient(90deg, #16a34a, #4ade80)"></div>
        </div>
    </div>

    <div class="stat-card anim-fade-up delay-5 rounded-2xl p-5 relative overflow-hidden text-white"
        ...
        <p class="text-xs font-semibold uppercase tracking-wider mb-1" style="color: rgba(255,255,255,0.6)">Event Aktif</p>
        <p class="text-3xl font-black">{{ number_format($totalEventAktif) }}</p>
        ...
        <div class="absolute -top-4 -right-8 w-16 h-16 rounded-full" style="background: rgba(74,158,255,0.1)"></div>
    </div>
...
<div class="grid grid-cols-5 gap-4 mb-4">
    ...
    {{-- Tren Mingguan --}}
    <div class="col-span-2 anim-fade-up delay-6 bg-white rounded-2xl p-5 border border-gray-100"
        style="box-shadow: 0 2px 12px rgba(0,0,0,0.04)">
        <div class="flex justify-between items-start mb-4">
            <div>
            ...
            <a href="{{ route('admin.laporan.index') }}"
                class="text-xs font-semibold hover:underline transition" style="color: #1a3a6b">
                Laporan →
            </a>
        </div>
        ...
        <div class="flex items-end gap-1.5 h-20 mb-2">
            ...
            <div class="flex-1 flex flex-col items-center gap-1 group">
                <div class="w-full rounded-t-lg transition-all duration-500 relative"
                    ...
                    @if($count > 0)
                    <div class="absolute -top-5 left-1/2 -translate-x-1/2 text-xs font-bold opacity-0 group-hover:opacity-100 transition whitespace-nowrap"
                        style="color: #1a3a6b; font-size: 9px">
                        {{ $count }}
                    </div>
                    @endif
                </div>
    ...
    {{-- Recent Transactions --}}
    <div class="col-span-3 anim-fade-up delay-7 bg-white rounded-2xl border border-gray-100 overflow-hidden"
        style="box-shadow: 0 2px 12px rgba(0,0,0,0.04)">
        <div class="flex justify-between items-center px-5 py-4 border-b border-gray-50">
            ...
            <a href="{{ route('admin.laporan.export') }}"
                class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold text-white transition hover:opacity-90"
            ...
            <table class="w-full text-xs">
                <thead>
                    <tr style="background: #f8faff">
                        <th class="px-4 py-3 text-left font-semibold text-gray-400 uppercase tracking-wider" style="font-size: 10px">ID</th>
                        <th class="px-4 py-3 text-left font-semibold text-gray-400 uppercase tracking-wider" style="font-size: 10px">Penitip</th>
                        <th class="px-4 py-3 text-left font-semibold text-gray-400 uppercase tracking-wider" style="font-size: 10px">Barang</th>
                        <th class="px-4 py-3 text-left font-semibold text-gray-400 uppercase tracking-wider" style="font-size: 10px">Event</th>
                        <th class="px-4 py-3 text-left font-semibold text-gray-400 uppercase tracking-wider" style="font-size: 10px">Status</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr class="table-row border-t border-gray-50">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="font-medium text-gray-700">{{ Str::limit($t->nama_penitip, 12) }}</span>
                            </div>
                        </td>
                        <td class="px-4 py-3">
                            @foreach($t->details->take(1) as $d)
                            <span class="px-2 py-0.5 rounded-md text-gray-600 font-medium"
                                style="background: #f1f5f9; font-size: 10px">
                                {{ Str::limit($d->nama_barang_custom ?? $d->kategori->nama_kategori, 10) }}
                            </span>
                            @endforeach
                        </td>
                        <td class="px-4 py-3 text-gray-500">{{ Str::limit($t->event->nama_event, 12) }}</td>
                        <td class="px-4 py-3">
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="px-4 py-8 text-center text-gray-300">Belum ada transaksi.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="grid grid-cols-2 gap-x-8 gap-y-4">
        ...
        <div>
            <div class="flex justify-between items-center mb-1.5">
                ...
                <p class="text-sm font-black" style="color: {{ $colors[$i][0] }}">
