    ...
    <div class="flex flex-col sm:flex-row gap-4 mb-5">
        ...
        <div class="anim-fade-up delay-3 w-full sm:w-64 rounded-2xl flex flex-col items-center justify-center p-6 text-white flex-shrink-0"
            ...
            @if (isset($transaksis) && $transaksis->count() > 0)
                @if ($transaksis->where('status', 'terlambat')->count() > 0)
                    ...
                    <p class="text-xs text-center mt-1" style="color: #fca5a5">
                        Ada barang dengan status <strong>terlambat</strong>. Segera konfirmasi.
                    </p>
                @else
                    <div class="w-14 h-14 rounded-full flex items-center justify-center text-3xl mb-3"
            ...
            @else
                ...
                <p class="text-xs text-center mt-1" style="color: #c4b5fd">Masukkan nama penitip untuk mencari data barang.
                </p>
            @endif
        </div>
                    ...
                    <div class="anim-fade-up delay-4 rounded-2xl overflow-hidden border border-gray-100"
                        ...
                        <div class="bg-white px-6 py-6">
                            <div class="grid grid-cols-3 gap-6 mb-6">
                                ...
                                <div>
                                    <p class="text-xs font-semibold uppercase tracking-wider mb-1" style="color: #94a3b8">
                                        Quantity</p>
                                    <p class="text-xl font-black text-gray-800">{{ $transaksi->details->sum('jumlah') }} Unit
                                    </p>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-3 gap-6 mb-6">
                                <div>
                                    <p class="text-xs font-semibold uppercase tracking-wider mb-2" style="color: #94a3b8">Ukuran
                                    </p>
                                ...
                                <div>
                                    <p class="text-xs font-semibold uppercase tracking-wider mb-1" style="color: #94a3b8">Event
                                    </p>
                                ...
                                <div>
                                    ...
                                    @if ($transaksi->status === 'terlambat')
                                        <p class="flex items-center gap-2 font-bold" style="color: #dc2626">
                                            <span class="w-2 h-2 rounded-full animate-pulse inline-block bg-red-500"></span>
                                            Terlambat Diambil
                                        </p>
                                    @else
                            ...
                            <div>
                                <div class="flex justify-between items-center mb-2">
                                    <p class="text-xs font-semibold uppercase tracking-wider" style="color: #94a3b8">Durasi
                                        Penitipan</p>
                                    <p class="text-xs font-bold" style="color: #7c3aed">{{ $durasi }} Jam /
                                        {{ $maxJam }} Jam</p>
                                </div>
                                <div class="w-full bg-gray-100 rounded-full h-2">
                                    <div class="h-2 rounded-full transition-all"
                                        style="width: {{ $pctDurasi }}%; background: linear-gradient(to right, #5b21b6, #a78bfa)">
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="flex items-start gap-3 px-4 py-3 rounded-xl mb-3"
                                    ...
                                    <div>
                                        ...
                                        <p class="text-red-400 text-xs mt-0.5">
                                            Event sudah berakhir. Barang ini melewati batas waktu pengambilan.
                                            Konfirmasi tetap bisa dilakukan dengan catatan keterlambatan.
                                        </p>
                                    </div>
                                </div>
                            @endif
                            <form action="{{ route('kasir.pengambilan.konfirmasi', $transaksi) }}" method="POST"
                                onsubmit="return confirm('{{ $transaksi->status === 'terlambat' ? 'Barang ini terlambat diambil. Tetap konfirmasi?' : 'Konfirmasi pengambilan barang atas nama ' . $transaksi->nama_penitip . '?' }}')">
                                @csrf
                                <button type="submit"
                                    ...
                                    style="background: {{ $transaksi->status === 'terlambat' ? 'linear-gradient(135deg, #dc2626, #ef4444)' : 'linear-gradient(135deg, #5b21b6, #7c3aed)' }}; box-shadow: 0 4px 16px rgba(91,33,182,0.25)">
