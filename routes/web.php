...
Route::prefix('admin')->middleware(['auth', 'role:admin'])->name('admin.')->group(function () {
    ...
    // FIX #1: Hapus duplikasi Route::resource('events') — sebelumnya terdaftar 2x
...
Route::prefix('kasir')->middleware(['auth', 'role:kasir'])->name('kasir.')->group(function () {
    ...
    // Taruh count-today SEBELUM resource agar tidak terbentur route {transaksi}
    ...
    Route::post('/pengambilan/scan-qr', [PengambilanController::class, 'scanQr'])->name('pengambilan.scan-qr');
    ...
    Route::get('/profile', [App\Http\Controllers\Kasir\ProfileController::class, 'index'])->name('profile');
    Route::put('/profile', [App\Http\Controllers\Kasir\ProfileController::class, 'update'])->name('profile.update');
    Route::put('/profile/password', [App\Http\Controllers\Kasir\ProfileController::class, 'updatePassword'])->name('profile.password');
});
